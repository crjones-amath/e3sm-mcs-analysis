...
# add parser info if run from command line:
parser = argparse.ArgumentParser()
# parser.add_argument('--filein', '-f')
parser.add_argument('--date', '-d')
parser.add_argument('--model', '-m', default='e3sm')
...
if 'mmf' in model.lower():
    ...
    raise ValueError('Unknown model -- choose e3sm or mmf')
...
def subset_and_write_to_file(fname, lons=[220, 300], lats=[20, 50],
                             ...
                             transpose_dims=False, shift_time=True):
    print('Processing file ', fname)
